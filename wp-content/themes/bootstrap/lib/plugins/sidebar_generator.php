...
//		add_submenu_page('themes.php', 'Sidebars', 'Sidebars', 'manage_options', __FILE__, array('sidebar_generator','admin_page'));
	...
	*/
	public static function get_sidebar($name="0",$class=""){
        $test=0;		if ($test) _log('get sidebar');
		
		if(!is_singular()){
		    global $my_Admin;
		    $sidebarpos = $my_Admin->get_option_value('sidebar_pos');
			if ($test)  _log($sidebarpos.'  '.$name);
			    if($name != $sidebarpos){
				return;
			}else{
		...
		$did_sidebar = false;
_log('************************************************selected_sidebar');
//_log($selected_sidebar);
global $my_Pagesettings;		
global $my_Admin;

$sidebarpos = $my_Pagesettings->get_option_value('pagesidebar');
if ($sidebarpos==='Theme')
    	$sidebarpos = $my_Admin->get_option_value('sidebar_pos');

$sidebar = $my_Pagesettings->get_option_value('choosesidebar');
 if ($test) _log('sidebar====>'.$sidebar);

if ($sidebar==='Theme')
    	$sidebar = 'default'; //$my_Admin->get_option_value('sidebar_pos');
if ($test) _log('sidebar====>'.$sidebar);
if ($sidebarpos===$name) {
    if ($test) _log("lika med sidebar ska genereras");
...
_log("Sidebar class skapad");

function generated_dynamic_sidebar($name='0',$class="col-md-4"){
	sidebar_generator::get_sidebar($name,$class);	
	return true;
}
?>